<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="icon" href="../assets/img/logo/favicon.ico" type="image/gif" sizes="20x20">
    <title>Kali Adventure Camp, Dandeli – Explore Nature and Wildlife</title>
    <meta name="description" content="Plan your getaway to Kali Adventure Camp, Dandeli. Enjoy thrilling activities, scenic rivers, and wildlife in the heart of nature. Book your stay now.">
    <meta name="geo.region" content="IN">
    <link rel="canonical" href="https://www.thekarnatakatourism.com/eco-resorts/kali-adventure-camp.php">
    <?php include('../include/inner/common-link.php'); ?>
</head>

<body>
    <?php include('../include/inner/header.php'); ?>

    <div class="package-details-area pt-80 mb-40 position-relative">
        <div class="container">
            <div class="row">
                <div class="co-lg-12">
                    <div class="package-img-group mb-50">
                        <div class="row align-items-center g-3">
                            <div class="col-lg-6">
                                <div class="gallery-img-wrap">
                                    <img src="../assets/img//resorts/kali-resort-1.jpg" alt="Karnataka Tourism">
                                    <a data-fancybox="gallery-01" href="../assets/img/resorts/kali-resort-1.jpg"><i class="bi bi-eye"></i></a>
                                </div>
                            </div>
                            <div class="col-lg-6 h-100">
                                <div class="row g-3 h-100">
                                    <div class="col-6">
                                        <div class="gallery-img-wrap">
                                            <img src="../assets/img//resorts/kali-resort-2.jpg" alt="Karnataka Tourism">
                                            <a data-fancybox="gallery-01" href="../assets/img/resorts/kali-resort-2.jpg"><i class="bi bi-eye"></i></a>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="gallery-img-wrap">
                                            <img src="../assets/img//resorts/kali-resort-3.jpg" alt="Karnataka Tourism">
                                            <a data-fancybox="gallery-01" href="../assets/img/resorts/kali-resort-3.jpg"><i class="bi bi-eye"></i></a>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="gallery-img-wrap active">
                                            <img src="../assets/img/resorts/kali-resort-4.jpg" alt="Karnataka Tourism">
                                            <button class="StartSlideShowFirstImage"><i class="bi bi-plus-lg"></i> View More Images</button>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="gallery-img-wrap active">
                                            <img src="../assets/img/resorts/kali-resort-5.jpg" alt="Karnataka Tourism">
                                            <a data-fancybox="gallery-01" href="../assets/img/resorts/kali-resort-video.mp4"><i class="bi bi-play-circle"></i> Watch Video</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="others-image-wrap d-none">
                <a href="../assets/img/resorts/kali-resort-6.jpg" data-fancybox="images"><img src="../assets/img/resorts/kali-resort-6.jpg" alt="Karnataka Tourism"></a>
                <a href="../assets/img/resorts/kali-resort-7.jpg" data-fancybox="images"><img src="../assets/img/resorts/kali-resort-7.jpg" alt="Karnataka Tourism"></a>
                <a href="../assets/img/resorts/kali-resort-8.jpg" data-fancybox="images"><img src="../assets/img/resorts/kali-resort-8.jpg" alt="Karnataka Tourism"></a>
                <a href="../assets/img/resorts/kali-resort-9.jpg" data-fancybox="images"><img src="../assets/img/resorts/kali-resort-9.jpg" alt="Karnataka Tourism"></a>
                <a href="../assets/img/resorts/kali-resort-10.jpg" data-fancybox="images"><img src="../assets/img/resorts/kali-resort-10.jpg" alt="Karnataka Tourism"></a>
            </div>
            <div class="row g-xl-4 gy-5">
                <div class="col-xl-8">
                    <h1 class="eco-head">Welcome to Kali Adventure Camp, Dandeli!</h1>
                    <div class="tour-price">
                        <span>starts from </span>&nbsp;
                        <h3>₹2360.00/- </h3>
                        <span>per person</span>
                    </div>

                    <ul class="tour-info-metalist">
                        <li>
                            <svg width="14" height="14" viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
                                <path d="M14 7C14 8.85652 13.2625 10.637 11.9497 11.9497C10.637 13.2625 8.85652 14 7 14C5.14348 14 3.36301 13.2625 2.05025 11.9497C0.737498 10.637 0 8.85652 0 7C0 5.14348 0.737498 3.36301 2.05025 2.05025C3.36301 0.737498 5.14348 0 7 0C8.85652 0 10.637 0.737498 11.9497 2.05025C13.2625 3.36301 14 5.14348 14 7ZM7 3.0625C7 2.94647 6.95391 2.83519 6.87186 2.75314C6.78981 2.67109 6.67853 2.625 6.5625 2.625C6.44647 2.625 6.33519 2.67109 6.25314 2.75314C6.17109 2.83519 6.125 2.94647 6.125 3.0625V7.875C6.12502 7.95212 6.14543 8.02785 6.18415 8.09454C6.22288 8.16123 6.27854 8.2165 6.3455 8.25475L9.408 10.0048C9.5085 10.0591 9.62626 10.0719 9.73611 10.0406C9.84596 10.0092 9.93919 9.93611 9.99587 9.83692C10.0525 9.73774 10.0682 9.62031 10.0394 9.50975C10.0107 9.39919 9.93982 9.30426 9.842 9.24525L7 7.62125V3.0625Z"></path>
                            </svg>
                            2 Days / 1 Night
                        </li>
                        <li>
                            <svg width="14" height="14" viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
                                <path d="M7 7C7.92826 7 8.8185 6.63125 9.47487 5.97487C10.1313 5.3185 10.5 4.42826 10.5 3.5C10.5 2.57174 10.1313 1.6815 9.47487 1.02513C8.8185 0.368749 7.92826 0 7 0C6.07174 0 5.1815 0.368749 4.52513 1.02513C3.86875 1.6815 3.5 2.57174 3.5 3.5C3.5 4.42826 3.86875 5.3185 4.52513 5.97487C5.1815 6.63125 6.07174 7 7 7ZM14 12.8333C14 14 12.8333 14 12.8333 14H1.16667C1.16667 14 0 14 0 12.8333C0 11.6667 1.16667 8.16667 7 8.16667C12.8333 8.16667 14 11.6667 14 12.8333Z"></path>
                            </svg>
                            Max People : 4
                        </li>
                        <li>
                            <svg width="14" height="14" viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M14 0.43748C14 0.372778 13.9856 0.308889 13.9579 0.250418C13.9302 0.191947 13.8898 0.140348 13.8398 0.0993396C13.7897 0.0583312 13.7312 0.0289339 13.6684 0.0132656C13.6057 -0.00240264 13.5402 -0.00395173 13.4768 0.00872996L9.1875 0.86623L4.89825 0.00872996C4.84164 -0.00258444 4.78336 -0.00258444 4.72675 0.00872996L0.35175 0.88373C0.252608 0.903546 0.163389 0.957088 0.099263 1.03525C0.0351366 1.11342 6.10593e-05 1.21138 0 1.31248L0 13.5625C3.90711e-05 13.6272 0.0144289 13.6911 0.0421328 13.7495C0.0698367 13.808 0.110165 13.8596 0.160212 13.9006C0.210259 13.9416 0.268779 13.971 0.331556 13.9867C0.394332 14.0024 0.459803 14.0039 0.52325 13.9912L4.8125 13.1337L9.10175 13.9912C9.15836 14.0025 9.21664 14.0025 9.27325 13.9912L13.6482 13.1162C13.7474 13.0964 13.8366 13.0429 13.9007 12.9647C13.9649 12.8865 13.9999 12.7886 14 12.6875V0.43748ZM4.375 12.3287V0.97123L4.8125 0.88373L5.25 0.97123V12.3287L4.89825 12.2587C4.84165 12.2474 4.78335 12.2474 4.72675 12.2587L4.375 12.3287ZM8.75 13.0287V1.67123L9.10175 1.74123C9.15836 1.75254 9.21664 1.75254 9.27325 1.74123L9.625 1.67123V13.0287L9.1875 13.1162L8.75 13.0287Z"></path>
                            </svg>
                            Adventure & Wildlife
                        </li>
                    </ul>
                    <h3 class="mb-2" style="color:#63ab45">Your Gateway to Thrills in Nature's Embrace</h3>
                    <p>Dive into an unforgettable adventure at Kali Adventure Camp, where excitement and the beauty of nature come together in perfect harmony. Situated on the enchanting banks of the Kali River, our camp invites travellers to explore the vibrant forests and rich wildlife of the Western Ghats. Whether you're paddling through exhilarating white-water rapids or simply soaking in the tranquil jungle surroundings, our eco-friendly accommodations provide the ideal blend of comfort and adventure.</p>
                    <h3 class="mb-2" style="color:#63ab45">Thrills and Tranquility Await You!</h3>
                    <p>Every day at Kali Adventure Camp is packed with opportunities for exploration and excitement. Adrenaline junkies can tackle exhilarating activities like white-water rafting and trekking, while nature enthusiasts can embark on captivating wildlife safaris through Dandeli's diverse ecosystems. Prefer a more leisurely pace? Take a serene riverside stroll or enjoy birdwatching amidst the stunning backdrop of lush greenery. With a perfect mix of adventure and relaxation, Kali Adventure Camp guarantees an unforgettable experience.</p>
                    <p>Start planning your adventure today, and get ready to immerse yourself in the wild thrill of the great outdoors!</p>

                </div>
                <div class="col-xl-4">
                    <div class="contact-page">

                        <div class="single-contact mb-40">
                            <div class="title">
                                <h6>Phone</h6>
                            </div>
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
                                    <path d="M19.4752 15.4282L15.4271 12.7286C14.9134 12.3888 14.2248 12.5006 13.845 12.9856L12.6658 14.5017C12.5925 14.5983 12.4868 14.6653 12.3681 14.6903C12.2494 14.7153 12.1257 14.6966 12.0196 14.6377L11.7953 14.514C11.0518 14.1087 10.1266 13.6042 8.26252 11.7394C6.39846 9.87471 5.89287 8.94884 5.48757 8.20664L5.36458 7.98234C5.30488 7.87632 5.28549 7.75231 5.30998 7.63313C5.33448 7.51395 5.40121 7.40763 5.49789 7.33376L7.01297 6.15495C7.49775 5.77512 7.60972 5.08671 7.27028 4.57284L4.57068 0.524768C4.2228 0.00148647 3.52486 -0.1556 2.9863 0.168141L1.29355 1.18498C0.761668 1.49766 0.371466 2.00408 0.204723 2.59809C-0.404835 4.81909 0.0537292 8.6522 5.70024 14.2994C10.1919 18.7907 13.5358 19.9985 15.8341 19.9985C16.363 20.0008 16.8899 19.932 17.4005 19.7939C17.9947 19.6274 18.5012 19.2371 18.8136 18.7051L19.8315 17.0133C20.1557 16.4747 19.9987 15.7763 19.4752 15.4282ZM19.2572 16.6714L18.2417 18.3641C18.0173 18.7481 17.653 19.0305 17.2252 19.152C15.1748 19.7149 11.5927 19.2487 6.17181 13.8278C0.750931 8.40692 0.284711 4.82511 0.847635 2.77444C0.969353 2.34604 1.25202 1.98132 1.6365 1.75658L3.32926 0.741065C3.56294 0.60046 3.86586 0.668614 4.01682 0.89569L5.48328 3.09736L6.71411 4.94341C6.86151 5.16631 6.81308 5.46505 6.6028 5.62999L5.08736 6.8088C4.62618 7.16144 4.49448 7.79951 4.77842 8.30592L4.89876 8.52456C5.32471 9.30578 5.85428 10.2776 7.78766 12.2107C9.72105 14.1437 10.6926 14.6733 11.4734 15.0993L11.6924 15.2199C12.1988 15.5038 12.8369 15.3721 13.1895 14.911L14.3683 13.3955C14.5333 13.1854 14.8319 13.137 15.0549 13.2842L19.1026 15.9838C19.3299 16.1346 19.398 16.4377 19.2572 16.6714ZM11.3327 3.33435C14.4604 3.33783 16.9951 5.87249 16.9986 9.0002C16.9986 9.18427 17.1478 9.33347 17.3318 9.33347C17.5159 9.33347 17.6651 9.18427 17.6651 9.0002C17.6612 5.5045 14.8284 2.67164 11.3327 2.66781C11.1487 2.66781 10.9995 2.81701 10.9995 3.00108C10.9995 3.18516 11.1487 3.33435 11.3327 3.33435Z" />
                                    <path d="M11.3321 5.33353C13.3558 5.33592 14.9958 6.97591 14.9982 8.99967C14.9982 9.08806 15.0333 9.17283 15.0958 9.23533C15.1583 9.29783 15.2431 9.33294 15.3315 9.33294C15.4198 9.33294 15.5046 9.29783 15.5671 9.23533C15.6296 9.17283 15.6647 9.08806 15.6647 8.99967C15.662 6.60792 13.7238 4.66973 11.3321 4.66699C11.148 4.66699 10.9988 4.81619 10.9988 5.00026C10.9988 5.18434 11.148 5.33353 11.3321 5.33353Z" />
                                    <path d="M11.3321 7.33353C12.2519 7.33463 12.9974 8.08006 12.9985 8.99997C12.9985 9.08836 13.0336 9.17313 13.0961 9.23563C13.1586 9.29813 13.2434 9.33324 13.3318 9.33324C13.4201 9.33324 13.5049 9.29813 13.5674 9.23563C13.6299 9.17313 13.665 9.08836 13.665 8.99997C13.6635 7.71211 12.6199 6.66844 11.3321 6.66699C11.148 6.66699 10.9988 6.81619 10.9988 7.00026C10.9988 7.18434 11.148 7.33353 11.3321 7.33353Z" />
                                </svg>
                            </div>
                            <div class="content">
                                <h6><a href="tel:+919449244567">094492-44567</a></h6>
                            </div>
                        </div>

                        <div class="single-contact mb-40">
                            <div class="title">
                                <h6>Email us</h6>
                            </div>
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
                                    <path d="M19.9018 8.6153C19.5412 5.99522 18.1517 3.62536 16.0393 2.02707C13.9268 0.428777 11.2643 -0.267025 8.63745 0.0927308C6.01063 0.452486 3.63468 1.83833 2.03228 3.94539C0.42988 6.05245 -0.267711 8.70813 0.0929693 11.3282C0.388972 13.4966 1.38745 15.509 2.9363 17.0589C4.48516 18.6088 6.49948 19.6113 8.67243 19.9136C9.11786 19.9713 9.56656 20.0002 10.0157 20C11.8278 20.0033 13.606 19.5101 15.1563 18.5744C15.2358 18.5318 15.3058 18.4735 15.362 18.4031C15.4182 18.3326 15.4595 18.2516 15.4833 18.1648C15.5072 18.078 15.5131 17.9872 15.5007 17.8981C15.4884 17.8089 15.458 17.7232 15.4114 17.6461C15.3648 17.569 15.303 17.5021 15.2298 17.4496C15.1565 17.397 15.0733 17.3599 14.9853 17.3403C14.8972 17.3208 14.806 17.3193 14.7173 17.336C14.6287 17.3527 14.5443 17.3871 14.4694 17.4373C12.7129 18.4904 10.6392 18.8886 8.61629 18.5613C6.59339 18.2339 4.75224 17.2022 3.4197 15.6492C2.08717 14.0962 1.34948 12.1225 1.3376 10.0784C1.32573 8.03438 2.04043 6.05225 3.35483 4.48397C4.66923 2.91568 6.49828 1.86271 8.51723 1.512C10.5362 1.16129 12.6144 1.53554 14.383 2.56829C16.1515 3.60104 17.4959 5.22548 18.1776 7.1532C18.8592 9.08092 18.8338 11.1872 18.1061 13.0981C17.9873 13.4102 17.7626 13.6709 17.4711 13.8349C17.1795 13.999 16.8396 14.056 16.5104 13.996C16.1811 13.9361 15.8833 13.763 15.6687 13.5068C15.454 13.2506 15.3362 12.9275 15.3356 12.5936V5.37867C15.3356 5.2024 15.2654 5.03336 15.1404 4.90872C15.0155 4.78408 14.846 4.71406 14.6693 4.71406C14.4925 4.71406 14.3231 4.78408 14.1981 4.90872C14.0731 5.03336 14.0029 5.2024 14.0029 5.37867V6.52578C13.2819 5.70734 12.3261 5.12961 11.265 4.8708C10.204 4.61198 9.08877 4.68456 8.0704 5.07873C7.05203 5.47289 6.17966 6.16961 5.57134 7.07458C4.96303 7.97954 4.64814 9.04908 4.66929 10.1384C4.69045 11.2278 5.04663 12.2843 5.68962 13.1651C6.33262 14.0459 7.23139 14.7084 8.2643 15.0629C9.2972 15.4175 10.4144 15.4469 11.4646 15.1473C12.5149 14.8477 13.4475 14.2335 14.1362 13.3878C14.3015 13.9385 14.6358 14.4237 15.092 14.775C15.5482 15.1263 16.1033 15.326 16.6793 15.3461C17.2553 15.3662 17.8231 15.2057 18.3028 14.887C18.7825 14.5684 19.15 14.1078 19.3535 13.5699C19.9483 11.99 20.1368 10.2866 19.9018 8.6153ZM10.0051 14.0185C9.21436 14.0185 8.4414 13.7847 7.78396 13.3465C7.12651 12.9083 6.61409 12.2856 6.3115 11.5569C6.00891 10.8283 5.92974 10.0265 6.08399 9.25296C6.23825 8.47943 6.61902 7.7689 7.17813 7.21122C7.73724 6.65354 8.4496 6.27376 9.22511 6.1199C10.0006 5.96603 10.8045 6.045 11.535 6.34681C12.2655 6.64863 12.8899 7.15973 13.3292 7.8155C13.7685 8.47126 14.0029 9.24223 14.0029 10.0309C14.0019 11.0882 13.5803 12.1018 12.8308 12.8494C12.0813 13.597 11.065 14.0175 10.0051 14.0185Z" />
                                </svg>
                            </div>
                            <div class="content">
                                <h6><a href="mailto:book@thekarnatakatourism.com">book@thekarnatakatourism.com</a></h6>
                            </div>
                        </div>

                        <div class="single-contact mb-40">
                            <div class="title">
                                <h6>Booking Office</h6>
                            </div>
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="20" viewBox="0 0 16 20">
                                    <path d="M13.1781 11.175C12.4989 12.5025 11.5785 13.825 10.6374 15.0125C9.74465 16.132 8.79005 17.2044 7.77778 18.225C6.76548 17.2044 5.81089 16.132 4.91815 15.0125C3.97704 13.825 3.05667 12.5025 2.37741 11.175C1.69037 9.83375 1.2963 8.5775 1.2963 7.5C1.2963 5.8424 1.97916 4.25269 3.19468 3.08058C4.41019 1.90848 6.05878 1.25 7.77778 1.25C9.49677 1.25 11.1454 1.90848 12.3609 3.08058C13.5764 4.25269 14.2593 5.8424 14.2593 7.5C14.2593 8.5775 13.8639 9.83375 13.1781 11.175ZM7.77778 20C7.77778 20 15.5556 12.8925 15.5556 7.5C15.5556 5.51088 14.7361 3.60322 13.2775 2.1967C11.8189 0.790176 9.84057 0 7.77778 0C5.71498 0 3.73667 0.790176 2.27806 2.1967C0.819442 3.60322 3.0738e-08 5.51088 0 7.5C0 12.8925 7.77778 20 7.77778 20Z" />
                                    <path d="M7.77951 10C7.09192 10 6.43248 9.73661 5.94627 9.26777C5.46007 8.79893 5.18692 8.16304 5.18692 7.5C5.18692 6.83696 5.46007 6.20107 5.94627 5.73223C6.43248 5.26339 7.09192 5 7.77951 5C8.46711 5 9.12655 5.26339 9.61275 5.73223C10.099 6.20107 10.3721 6.83696 10.3721 7.5C10.3721 8.16304 10.099 8.79893 9.61275 9.26777C9.12655 9.73661 8.46711 10 7.77951 10ZM7.77951 11.25C8.81091 11.25 9.80007 10.8549 10.5294 10.1517C11.2587 9.44839 11.6684 8.49456 11.6684 7.5C11.6684 6.50544 11.2587 5.55161 10.5294 4.84835C9.80007 4.14509 8.81091 3.75 7.77951 3.75C6.74812 3.75 5.75896 4.14509 5.02965 4.84835C4.30035 5.55161 3.89062 6.50544 3.89062 7.5C3.89062 8.49456 4.30035 9.44839 5.02965 10.1517C5.75896 10.8549 6.74812 11.25 7.77951 11.25V11.25Z" />
                                </svg>
                            </div>
                            <div class="content">
                                <h6>
                                    Kogilban, Haliyal Taluk,<br>
                                    Uttar Kannada District<br>
                                    Dandeli, Around Karwar - 581325.<br>
                                    Karnataka, India.
                                </h6>
                            </div>
                        </div>

                    </div>

                </div>
                <div class="col-lg-12 d-flex justify-content-center mt-0">
                    <a href="../book.php" class="primary-btn4 two" style="width:100%">
                        <span style="width:100%">
                            Book Now
                            <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17">
                                <path d="M7.70294 9.65818L7.27464 11.6673L5.29549 17.0003L6.47016 16.8046L11.9577 9.62856C12.7321 9.6016 13.4832 9.56006 14.1359 9.49563C17.1552 9.19702 16.9986 8.50013 16.9986 8.50013C16.9986 8.50013 17.1552 7.80325 14.1358 7.50464C13.4832 7.43995 12.7321 7.39845 11.9576 7.3717L6.47019 0.195477L5.29549 -5.1162e-07L7.27464 5.33303L7.70294 7.34212C6.69752 7.35717 6.01715 7.38006 6.01715 7.38006C6.01715 7.38006 4.63017 7.41207 2.48417 7.8956L0.734503 5.46859L-8.41624e-05 5.46859L0.523018 8.41949C0.428867 8.44835 0.428867 8.55195 0.523018 8.58081L-8.44274e-05 11.5317L0.734502 11.5317L2.48417 9.10495C4.63017 9.58848 6.01715 9.62001 6.01715 9.62001C6.01715 9.62001 6.69752 9.64317 7.70294 9.65818Z"></path>
                                <path d="M11.4004 11.2692L11.4004 12.0613L8.47265 12.0613L8.47265 11.2692L11.4004 11.2692ZM11.4004 4.94234L11.4004 5.73425L8.47282 5.73425L8.47282 4.94234L11.4004 4.94234ZM9.42515 13.9276L9.42515 14.7195L6.71923 14.7195L6.71923 13.9276L9.42515 13.9276ZM9.42515 2.28418L9.42515 3.07634L6.71924 3.07634L6.71924 2.28418L9.42515 2.28418Z"></path>
                            </svg>
                        </span>
                    </a>
                </div>

                <div class="col-xl-12">
                    <div class="room-details-area">
                        <h4>Amenities</h4>
                        <ul class="room-features">
                            <li>
                                <i class="fa-solid fa-wifi" style="color:#63ab45"></i>
                                Wifi
                            </li>
                            <li>
                                <i class="fa-solid fa-person-shelter" style="color:#63ab45"></i>
                                Living Room
                            </li>
                            <li>
                                <i class="fa-solid fa-couch" style="color:#63ab45"></i>
                                Seating Area
                            </li>
                            <li>
                                <i class="fa-solid fa-toilet" style="color:#63ab45"></i>
                                Private Bathroom
                            </li>
                            <li>
                                <i class="fa-solid fa-broom" style="color:#63ab45"></i>
                                Housekeeping
                            </li>
                            <li>
                                <i class="fa-solid fa-mug-hot" style="color:#63ab45"></i>
                                Coffee Maker
                            </li>
                            <li>
                                <i class="fa-solid fa-person-walking-luggage" style="color:#63ab45"></i>
                                Luggage Assiatance
                            </li>
                            <li>
                                <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M7.79309 3.86133C7.5763 3.91992 7.26575 4.04297 7.08997 4.13672C6.69739 4.34766 6.16419 4.86914 5.95911 5.23828C5.76575 5.58984 5.56653 6.2168 5.56653 6.48047V6.67969H4.67005H3.77942L3.56263 6.50391C3.15833 6.16992 2.73059 6.01758 2.20325 6.01758C1.55872 6.01758 1.13684 6.18164 0.697391 6.60352C-0.708859 7.94531 0.228641 10.3125 2.16223 10.3125C2.74231 10.3125 3.21692 10.1367 3.6095 9.77344L3.79114 9.60352L4.66419 9.62109L5.53723 9.63867L5.55481 16.623L5.56653 23.6074L5.71888 23.7246C5.87122 23.8418 5.94739 23.8477 7.44739 23.8477H9.02356V24.8086C9.02356 25.8457 9.07044 26.0801 9.29309 26.1855C9.39856 26.2324 11.4786 26.25 17.0158 26.25C24.5919 26.25 24.5919 26.25 24.7501 26.127L24.9025 26.0098L24.9142 17.8242L24.9318 9.63867L25.5704 9.62109C26.2033 9.60352 26.215 9.60938 26.3908 9.77344C26.7833 10.1367 27.2579 10.3125 27.838 10.3125C28.84 10.3125 29.6779 9.63867 29.924 8.64844C30.1056 7.88672 29.8595 7.11328 29.256 6.5625C28.8341 6.17578 28.4122 6.01758 27.797 6.01758C27.2697 6.01758 26.8419 6.16992 26.4376 6.50391C26.2267 6.67383 26.2091 6.67969 25.5646 6.67969H24.9025V6.48633C24.9025 6.22266 24.7501 5.72461 24.545 5.32617C24.3341 4.9043 23.7833 4.35352 23.3439 4.11914C22.6056 3.73242 22.963 3.75 15.1993 3.75586C8.672 3.75586 8.14466 3.76172 7.79309 3.86133ZM19.5939 4.9043C19.3302 5.25 19.1954 5.51953 19.0783 5.91797C19.0021 6.1875 18.9845 6.70898 18.9728 9.05859C18.9552 11.7656 18.9611 11.8887 19.0665 12.0059C19.1954 12.1523 19.4943 12.1699 19.6642 12.0469C19.7638 11.9707 19.7755 11.8652 19.8048 10.7988L19.8341 9.63867L21.9318 9.62109L24.0236 9.60938V17.4902V25.3711H16.963H9.90247V24.6094V23.8477H14.7013C19.465 23.8477 19.4943 23.8477 19.6525 23.7246L19.8048 23.6074V18.4512C19.8048 13.377 19.8048 13.3008 19.6876 13.1836C19.5294 13.0254 19.213 13.0312 19.0665 13.1895C18.9611 13.3066 18.9552 13.4531 18.9376 16.0605L18.92 18.8086H12.6857H6.44544V12.5508V6.29883L6.5802 5.9707C6.75598 5.54297 6.99036 5.25586 7.36536 5.00977C7.93958 4.62305 7.69934 4.63477 14.045 4.63477L19.8048 4.62891L19.5939 4.9043ZM22.4591 4.7168C23.2618 4.92773 23.924 5.64844 24.0118 6.42188L24.0411 6.67969H21.9318H19.8224L19.8693 6.39844C19.922 6.04688 20.0392 5.77734 20.2618 5.47266C20.5431 5.08594 20.8243 4.91016 21.381 4.76367C21.9904 4.60547 22.0372 4.60547 22.4591 4.7168ZM2.56067 6.94336C3.26966 7.1543 3.65052 8.02734 3.33411 8.70117C3.09388 9.20508 2.65442 9.45703 2.06848 9.42188C1.58216 9.39258 1.26575 9.17578 1.04309 8.73633C0.761844 8.18555 0.849735 7.6875 1.28919 7.25977C1.68177 6.87891 2.03919 6.79102 2.56067 6.94336ZM28.4064 7.02539C28.8341 7.24219 29.1212 7.72266 29.1212 8.21484C29.1212 8.52539 28.84 9.02344 28.547 9.22852C28.3185 9.38672 28.2482 9.4043 27.838 9.4043C27.4396 9.4043 27.3517 9.38672 27.1408 9.24023C26.4376 8.77734 26.338 7.83398 26.9415 7.24805C27.3458 6.84961 27.8966 6.76758 28.4064 7.02539ZM5.55481 8.15625L5.57239 8.73047H4.93958H4.30677V8.14453V7.55273L4.922 7.57031L5.53723 7.58789L5.55481 8.15625ZM25.6935 8.14453V8.73047H22.7521H19.8048V8.18555C19.8048 7.88086 19.8224 7.61719 19.8458 7.59961C19.8634 7.57617 21.1876 7.55859 22.7872 7.55859H25.6935V8.14453ZM18.9259 20.2734V20.8008H12.6857H6.44544V20.2734V19.7461H12.6857H18.9259V20.2734ZM18.9259 22.3242V22.9688H12.6857H6.44544V22.3242V21.6797H12.6857H18.9259V22.3242Z"></path>
                                </svg>
                                Fresh Towels
                            </li>
                            <li>
                                <i class="fa-solid fa-phone-volume" style="color:#63ab45"></i>
                                Wake-Up Call
                            </li>
                            <li>
                                <i class="fa-solid fa-car" style="color:#63ab45"></i>
                                Free Parking
                            </li>
                        </ul>
                    </div>
                    <div class="row">
                        <h4>Package Costing</h4>
                        <div class="col-lg-6 d-none d-lg-block" style="border: 1px solid #ddd"></div>
                        <div class="col-lg-6" style="border: 1px solid #ddd">
                            <h5 class="text-center py-1">Price in ₹ ( INR )</h5>
                        </div>
                        <div class="col-6" style="border: 1px solid #ddd">
                            <h6 class="py-1 text-center">Accomodation</h6>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <h6 class="py-1 text-center">Weekday</h6>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <h6 class="py-1 text-center">Weekend</h6>
                        </div>
                        <div class="col-6" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">Special Room</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">5050.00</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">5900.00</p>
                        </div>
                        <div class="col-6" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">Standard Room</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">4620.00</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">5430.00</p>
                        </div>
                        <div class="col-6" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">River View Tent</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">4620.00</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">5430.00</p>
                        </div>
                        <div class="col-6" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">Regular Tent</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">4520.00</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">5310.00</p>
                        </div>
                        <div class="col-6" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">Dormitory</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">2360.00</p>
                        </div>
                        <div class="col-3" style="border: 1px solid #ddd">
                            <p class="py-1 text-center">2360.00</p>
                        </div>
                    </div>
                    <p class="mt-15"><strong>Package Includes:</strong> Stay, Buffet Lunch Dinner & Breakfast with Jeep Safari into Kali Tiger Reserve, Coracle Ride, Guided Nature Walk, Forest Entry Fees, Camera Fees & Campfire.</p>
                </div>
                <div class="col-xl-12">
                    <div class="tour-plan">
                        <h3 class="tour-details-subtitle">Itinerary</h3>
                        <div class="tour-plan-single">
                            <div class="plan-serial">
                                <span>01</span>
                            </div>
                            <div class="plan-content">
                                <h4 class="plan-title">Day 01</h4>
                                <table class="event-table">
                                    <tbody>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>1:00 pm - </strong></td>
                                            <td class="event-details">
                                                Check in
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>1:30 pm - </strong></td>
                                            <td class="event-details">
                                                Buffet Lunch
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>3:30 pm - </strong></td>
                                            <td class="event-details">
                                                Our naturalists take you on a wildlife Safari in Vehicle in to the Kali Tiger Reserve / Coracle Ride in Kali River.
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>6:30 pm - </strong></td>
                                            <td class="event-details">
                                                Tea/ coffee served at the Gol Ghar.
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>7.30 pm - </strong></td>
                                            <td class="event-details">
                                                Watch a wildlife movie at the Conference Hall.
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>8:30 pm - </strong></td>
                                            <td class="event-details">
                                                Buffet Dinner
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                        <div class="tour-plan-single">
                            <div class="plan-serial">
                                <span>02</span>
                            </div>
                            <div class="plan-content">
                                <h4 class="plan-title">Day 02</h4>
                                <table class="event-table">
                                    <tbody>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>5:45 am - </strong></td>
                                            <td class="event-details">
                                                Wake-up call, tea/coffee
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>7:00 am - </strong></td>
                                            <td class="event-details">
                                                Our naturalists take you on a Nature walk / Bird watching.
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>8:30 am - </strong></td>
                                            <td class="event-details">
                                                Breakfast back at the camp
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>9:00 am - </strong></td>
                                            <td class="event-details">
                                                Coracle Ride
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><i class="bi bi-clock" style="color: #63AB45;"></i> <strong>10:30 am - </strong></td>
                                            <td class="event-details">
                                                Check-out
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xl-12">
                    <div class="row">
                        <div class="col-xl-12">
                            <h4>Note</h4>
                            <div class="includ-and-exclud-area mb-20">
                                <ul>
                                    <li><i class="bi bi-check-lg"></i> Tariffs are per person per night on twin-sharing basis.</li>
                                    <li><i class="bi bi-check-lg"></i> Transfers to & from our resorts are not included in the tariff.</li>
                                    <li><i class="bi bi-check-lg"></i> Single occupancy is charged at 30% of the tariff per night.</li>
                                    <li><i class="bi bi-check-lg"></i> Tariff for children (accompanying parents) between 5 and 12 years is 50%.</li>
                                    <li><i class="bi bi-check-lg"></i> Tariff May be changed without prior notice.</li>
                                    <li><i class="bi bi-check-lg"></i> Guest will bear the camera & lense entry fees.</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-xl-12">
                            <h4>Things To Do</h4>
                            <div class="includ-and-exclud-area mb-20">
                                <ul>
                                    <li><i class="bi bi-check-lg"></i> Visit Syntheri Rock, the 300-feet monolithic granite structure in the Dandeli Wildlife Sanctuary. The rock stands beside the beautiful River Kaneri.</li>
                                    <li><i class="bi bi-check-lg"></i> Visit Supa Dam about 25 kilometers away.</li>
                                    <li><i class="bi bi-check-lg"></i> Get close to wildlife again at Anshi National Park.</li>
                                    <li><i class="bi bi-check-lg"></i> Sykes Point is a popular sightseeing destination in the vicinity.</li>
                                    <li><i class="bi bi-check-lg"></i> Just about 22 kilometers away is the historic Shivaji Fort. This ancient fort is believed to have been built by the legendary Chatrapati Shivaji.</li>
                                    <li><i class="bi bi-check-lg"></i> At a distance of 25 kilometers is Kavala Caves. Test your stamina as you climb 375 steps to reach this destination. You will find naturally formed Shiva Lingam here.</li>
                                    <li><i class="bi bi-check-lg"></i> Visit the Sri Mallikarjuna Temple. It is believed that this temple came into existence somewhere in 12th century.</li>
                                    <li><i class="bi bi-check-lg"></i>If camping is your kind of thing, then don’t give Molangi a miss. The place is situated by the River Kali and is surrounded by huge rock formations.</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-xl-12">
                            <div class="tour-location">
                                <h4>How To Reach</h4>
                                <p><strong>By Train ?</strong> From Bangalore or Bombay, alight at Londa or Dharwad/ Hubli Railway Station and then proceed by road. A one-and-a-half hour journey will bring you to Dandeli.</p>
                                <p><strong>By Road ?</strong> Dandeli is well connected to Bangalore, Bombay, Goa, Belgaum, Karwar & Dharwad/Hubli by road. Several luxury buses ply to Dandeli everyday. There are direct overnight buses from Bangalore to Dandeli. The luxury buses plying between Mumbai and Mangalore / Bangalore, invariably pass through Dharwad. You can get down at Dharwad and take a taxi / bus to Dandeli.</p>
                                <p><strong>By Air ?</strong> Goa is about two and half hours from Dandeli and is well-connected by air. A few airlines have started flights to Hubli, from where Dandeli is an hour-and-half drive away.</p>
                                <div class="map-area mb-30">
                                    <iframe width="100%" height="450" frameborder="0" style="border:0" src="https://www.google.com/maps/embed?pb=!1m24!1m8!1m3!1d1980788.7897791746!2d75.0860492!3d14.1520761!3m2!1i1024!2i768!4f13.1!4m13!3e0!4m5!1s0x3bae1670c9b44e6d%3A0xf8dfc3e8517e4fe0!2sBengaluru%2C%20Karnataka!3m2!1d12.9715987!2d77.5945627!4m5!1s0x3bbedf896913fba3%3A0x49f9e99a138b95bb!2sKali%20Adventure%20Camp%2C%20Dandeli%2C%20Karnataka!3m2!1d15.240333199999998!2d74.6230794!5e0!3m2!1sen!2sin!4v1585042561901!5m2!1sen!2sin" allowfullscreen=""></iframe>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>

</body>
<?php include('../include/inner/footer.php'); ?>

</html>