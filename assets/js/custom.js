(function ($) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    mobileMenuButton.addEventListener("click", function () {
      ...
      if (mainMenu.classList.contains("scrollable")) {
        mainMenu.classList.remove("scrollable");
      } else {
        mainMenu.classList.add("scrollable");
      }
    });
  });
  ...
  $(function () {
    var today = moment().startOf("day"); // Get today's date and set time to midnight

    $('input[id="inDate"]').daterangepicker(
      {
        singleDatePicker: true,
        showDropdowns: true,
        minDate: today, // Disable the day before today
        minYear: 2024,
        maxYear: 2030,
        locale: {
          format: "DD-MMM-YYYY",
  ...
  $(function () {
    var today = moment().startOf("day"); // Get today's date and set time to midnight
    var startDate = moment().add(2, "days"); // Set start date as 2 days later

    $('input[id="outDate"]').daterangepicker(
      {
        singleDatePicker: true,
        showDropdowns: true,
        minDate: today, // Disable the day before today
        ...
        startDate: startDate, // Set start date as 2 days later
      },
      function (start, end, label) {
        var years = moment().diff(start, "years");
      }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    document
      ...
      .addEventListener("submit", function (event) {
        ...
        var resort = formData.get("resort");
        if (!resort) {
          errors.push("Please select a resort.");
          document.querySelector(".error-resort").style.display = "block";
        } else {
          document.querySelector(".error-resort").style.display = "none";
        }

        var arrivalDate = formData.get("arrival_date");
        if (!arrivalDate) {
          errors.push("Please choose your arrival date.");
          document.querySelector(".error-arrival-date").style.display = "block";
        } else {
          document.querySelector(".error-arrival-date").style.display = "none";
        }

        var departureDate = formData.get("departure_date");
        if (!departureDate) {
          errors.push("Please choose your departure date.");
          document.querySelector(".error-departure-date").style.display =
            "block";
        } else {
          document.querySelector(".error-departure-date").style.display =
            "none";
        ...
        var adult = formData.get("adult");
        if (!adult || isNaN(adult) || adult < 1) {
          errors.push("Please enter a valid number of adults.");
          document.querySelector(".error-adult").style.display = "block";
        } else {
          document.querySelector(".error-adult").style.display = "none";
        }
